...
```{r}
data_aggregated %>% 
  ggplot(aes(x = time_to_dead, y = dbh_mean)) +
  geom_jitter(width = 1)
```
...
surv_pred <- 
  data_aggregated %>% 
  data_grid(
    dbase_mean_sc = seq_range(dbase_mean, n = 5, pretty = TRUE),
    dbh_mean_sc = seq_range(dbase_mean, n = 5, pretty = TRUE), 
    forest_logged, 
    .model = survival_model) %>% 
  add_epred_draws(survival_model, 
                  re_formula = NA,
                  dpar = TRUE) %>% 
  filter(.category == "timetolastalive")
```
...
surv_pred %>% 
  ggplot(aes(x = dbh_mean_sc, y = .epred, 
               colour = forest_logged, 
               fill = forest_logged)) +
  stat_lineribbon()
```
