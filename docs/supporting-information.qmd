...
# Survival analysis of seedlings in the first census of the logged forest

57% of individuals were never recorded as alive (5,868 out of 10,272) 
i.e., they died in the period between planting and the first census. 
This occurs exclusively in the logged forest, 
since diameter measurements were not taken at the time of planting. 
Between 18 and 20 months elapsed between planting into the forest and censusing.
We excluded these individuals from the main analysis since they 
lack data on size and their mortality is likely due to stress from transplanting rather than any effect of forest type.
Nevertheless, this data might provide some insight on which species are more likely to survive transplanting from the nursery to the forest, 
and we present a simple analysis here.

To assess survival from planting to the first census in the logged forest, we fit a model in the `R` package `brms` 
with a Bernoulli response distribution, 
where zero indicated the seedling was dead in the first census and one indicated it was alive. 
The predictor was a fixed effect of species. 
We use a regularising prior of `normal(0, 2)`.

Results are presented in @fig-cens1-surv. 
Two of the 15 species were more than 50% likely to survive 
the first 18 to 20 months (_Shorea ovalis_ and _Hopea sangal_), 
four species were as equally likely to die as survive 
(_Shorea johorensis_, _Parashorea tomentella_, _parashorea malaanonan_ and _Dipterocarpus conformis_),
and the remaining nine species were more likely to die than survive.

![Posterior estimates of survival probability in the time period between planting and the first census of the logged forest. The posterior median is represented by a point with the line showing the 66% and 95% credible interval. The dashed red line shows where the likelihood of survival is 50% ](../code/notebooks/figures/2025-09-30_never-alive-seedlings/unnamed-chunk-7-1.png){#fig-cens1-surv width=400}
...
#| tbl-cap: "Posterior estimates of fixed effects from the survival model"
